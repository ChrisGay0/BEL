...
public class WelcomeLetter {
	...
	public String getSectionTwoText(Child child){
		return "If you wish to come for a 'Stay and Play' session before your child starts please contact the office to make a mutually convenient date and time. " +
				...
				"<br/>You will be invoiced on your childs first day at Burwell Early Learners (BEL) for the fees plus a £" + this.school.getDepositAmount() + " deposit which is refunded when you are eligible for " +
				"government funding and a £" + this.school.getRegistrationFee() + " non-refundable registration fee, if not previously paid.";
	}
